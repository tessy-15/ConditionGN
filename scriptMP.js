...
function getSlideSrc(slideNumber) {
  ...
  if (slideNumber === 12) {
    return 'slides/graph-positive-instruction.png';
  }
...
function updateOverlayMetrics() {
  ...
  [qualityOverlay, questionPageOverlay, photoFrameOverlay, scaleTableOverlay, validationOverlay, inputOverlay, radioOverlay, coverOverlay, backOverlay, finishOverlay].forEach((overlay) => {
...
function isForwardButtonPoint(event) {
  ...
  if (getCurrentSlideNumber() === 13) {
    // The graph's confirmation frame spans the same normalized image coordinates.
...
slideArea.addEventListener('mouseleave', () => {
  slideArea.classList.remove('forward-button-hover');
});
...
async function submitSurveyToGoogleForm() {
  ...
  try {
    await fetch(
      "https://docs.google.com/forms/d/e/1FAIpQLSfasUIlOtk-4Uyr6KeTdYd7WbHskpB7Wfqc6ng3naOz55-Cbg/formResponse",
      {
        method: "POST",
        mode: "no-cors",
        body: formData
